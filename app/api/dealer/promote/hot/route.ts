...
const HOT_DEAL_DURATION_MS = 48 * 60 * 60 * 1000;
...
export const POST = withApiHandler(async (request: NextRequest): Promise<NextResponse> => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const hotDealUntil = new Date(Date.now() + HOT_DEAL_DURATION_MS);
    ...
    if (error instanceof Error && error.message === "NO_CREDITS") {
      return apiError("No Hot Deal credits available", 400);
    }
    throw error;
  }
